...
public class MethodTransformationFactory {
	...
	public void initialize() {
		...
		TypeTransformationBuilder<Method, ?> methodBuilder = transformationBuilderFactory.create(Method.class,
				subRepository.createBuilder("editor.method"));
		...
		methodBuilder.transformCollection().from("params").convert(editorTransformationFactory.getTransformation());
		methodBuilder.transformCollection().from("pathVariables").convert(editorTransformationFactory.getTransformation());
		methodBuilder.transform().from("requestBody").convert(editorTransformationFactory.getTransformation());
		EntityTypeTransformation<Method, ?> methodTransformation = methodBuilder.buildTypeTransformation();
 
		TypeTransformationBuilder<Meta, ?> metaBuilder = transformationBuilderFactory.create(Meta.class,
				subRepository.createBuilder("editor.service"));
