...
public class DocBuilder
{
	...
	public EntityTypeTransformation<Type, ?> createTypeRefTransformation()
	{
		...
		entityTypeRefTransformationBuilder.transform()
			.from("@" + BindingMetaAttributeRegistrar.BINDING + ".externalTypeCode").to("name");
	...
	private Converter<Object, Object> getAttributeTransformation(EntityTypeTransformation typeRefTransformation)
	{
		...
		transformationBuilder.transform().from("@" + JavadocDataStore.META_ATTRIBUTE_CODE + ".description")
			.to("description");
	...
	public void init()
	{
		...
		entityTypeTransformationBuilder.transformCollection().from("declaredAttributes").to("attributes")
			.convert(getAttributeTransformation(typeRefTransformation));

		entityTypeTransformationBuilder.includeSuper(typeTransformation);

		entityTypeTransformationBuilder.transform()
			.from("@" + BindingMetaAttributeRegistrar.BINDING + ".externalTypeCode").to("name");
		entityTypeTransformationBuilder.transform().from("@" + JavadocDataStore.META_ATTRIBUTE_CODE + ".description")
			.to("description");
